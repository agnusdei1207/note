...
- **[CAP 이론](./nosql.md)**: NoSQL 설계의 근간이 되는 분산 시스템의 3요소 트레이드오프.
- **[샤딩(Sharding)](./nosql.md)**: 데이터를 여러 서버에 분산하여 수평적 확장을 구현하는 기술.
- **[폴리글랏 퍼시스턴스(Polyglot Persistence)](./nosql.md)**: 용도에 맞는 다양한 DB를 혼합하여 사용하는 아키텍처 전략.
- **[Eventual Consistency](./nosql.md)**: 분산 환경에서 시간이 지나면 결국 모든 노드의 데이터가 일치하게 되는 모델.
