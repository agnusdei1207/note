...
## 핵심 인사이트 (3줄 요약)
> **강화학습**은 에이전트가 환경과 상호작용하며 보상 신호를 통해 최적 행동 정책을 학습하는 ML 기법이다. AlphaGo·AlphaFold·ChatGPT의 RLHF·자율주행의 핵심 알고리즘이며, DQN→PPO→SAC→GRPO로 발전했다. PE 관점에서 **RLHF(LLM 정렬)·멀티에이전트 RL·로봇 학습**이 2024~2025 최신 트렌드다.
...
> **결론**: 강화학습은 "규칙 없이 경험으로 배우는 AI의 핵심 패러다임". ChatGPT 정렬(RLHF), AlphaGo, 자율주행, 바이오 등 다양한 분야의 혁신 엔진이다. PE는 마르코프 결정 과정·보상 함수 설계·RLHF 구성·멀티에이전트 RL을 이해해야 한다.
