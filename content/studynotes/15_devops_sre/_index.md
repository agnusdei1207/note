...
- [`[클라우드 네이티브 아키텍처]`](@/studynote/13_cloud_architecture/_index.md): DevOps 인프라의 꽃인 쿠버네티스(Kubernetes)와 마이크로서비스(MSA)의 본체.
- [`[소프트웨어 공학과 애자일]`](@/studynote/4_software_engineering/_index.md): DevOps 파이프라인의 논리적 뼈대이자 지속적 가치 제공의 근본 철학.
- [`[정보 보안 및 DevSecOps]`](@/studynote/9_security/_index.md): CI/CD 파이프라인에 시큐어 코딩 및 컨테이너 취약점 검사를 내재화하는 보안 생명주기.
- [`[운영체제와 가상화]`](@/studynote/2_operating_system/_index.md): Docker 컨테이너의 밑바탕이 되는 리눅스 커널 기술(cgroups, namespace)과 가상화 원리.
- [`[데이터 엔지니어링 파이프라인]`](@/studynote/14_data_engineering/_index.md): 애플리케이션 코드가 아닌 거대 데이터의 이동(ETL/ELT)을 CI/CD와 IaC로 제어하는 DataOps의 교집합.