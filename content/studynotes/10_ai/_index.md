...
- [`[딥러닝 기초 및 신경망]`](@/studynote/10_ai/2_dl_architecture/_index.md): 기계가 스스로 데이터의 특징을 추출하는 현대 AI의 절대적 기반.
- [`[자연어 처리와 RAG]`](@/studynote/10_ai/_index.md): 텍스트의 의미를 벡터로 분해하고 검색과 생성을 결합하는 환각 방어 기술.
- [`[GPU와 AI 하드웨어 가속기]`](@/studynote/1_computer_architecture/12_accelerators_ai/_index.md): 딥러닝의 천문학적 행렬 곱 연산을 물리적으로 감당하는 병렬 아키텍처.
- [`[빅데이터 분산 처리]`](@/studynote/16_bigdata/_index.md): 수백 테라바이트의 학습 데이터를 AI 모델에 먹여주기 위한 필수 데이터 파이프라인.
- [`[AI 윤리와 설명 가능한 AI(XAI)]`](@/studynote/10_ai/_index.md): 딥러닝 블랙박스의 한계를 극복하고 모델의 신뢰성을 인간의 언어로 증명하는 통제 기법.