...
### 비교 2: 상호작용 수준에 따른 허니팟 상태 트레이드오프
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **킬체인 (Kill Chain)** | 기만 기술이 개입하여 끊어내는 공격자의 단계별 행동 모델 (특히 횡적 이동 단계). |
| **MITRE ATT&CK** | 허니팟에서 수집된 공격자의 TTPs(전술, 기법, 절차)를 맵핑하여 방어 전략을 고도화하는 기준점. |
| **SDN (Software Defined Networking)** | 공격자 트래픽을 감지 즉시 유인망으로 동적, 투명하게 리다이렉트하는 네트워크 인프라 기반. |
| **제로 트러스트 (Zero Trust)** | 내부망에 들어온 자도 신뢰하지 않는다는 철학을 실천하기 위해, 신뢰할 수 없는 요청을 식별하는 디텍터로 기만 기술을 활용. |
| **SOAR (Security Orchestration, Automation and Response)** | 허니팟에서 알람이 발생하는 순간, 방화벽과 엔드포인트 EDR에 격리 명령을 자동 하달하는 오케스트레이션 엔진. |
