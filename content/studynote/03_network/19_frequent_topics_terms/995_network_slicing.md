...
weight = 995
[taxonomies]
tags = ["Network", "5G", "SDN", "NFV", "Slicing"]
+++

# 995. 네트워크 슬라이싱

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 네트워크 슬라이싱 (Network Slicing)은 단일 물리적 네트워크 인프라 위에, 요구사항(대역폭, 지연시간, 연결 밀도)이 극명하게 다른 여러 개의 독립적인 논리적 가상 네트워크(슬라이스)를 생성하고 격리하는 5G 통신의 핵심 아키텍처 기술이다.
> 2. **가치**: 기존의 경직된 단일 파이프 기반 통신망을 소프트웨어 중심의 맞춤형 서비스망으로 혁신함으로써, 통신사는 B2B 기업 고객에게 각 산업 특성에 맞는 전용망 (Private Network)을 구축비용 없이 SLA (Service Level Agreement) 보장 형태로 판매(NaaS)할 수 있게 되었다.
> 3. **융합**: 이 기술은 제어와 전송을 분리하는 SDN (Software Defined Networking)과 하드웨어를 가상화하는 NFV (Network Functions Virtualization) 기반 위에서 동작하며, 클라우드 네이티브 아키텍처 및 엣지 컴퓨팅(MEC)과 결합하여 E2E (End-to-End) 오케스트레이션을 완성한다.
...
### 비교 1: 기존 QoS 기반 트래픽 제어 vs 5G 네트워크 슬라이싱
...
### 비교 2: VPN (Virtual Private Network) vs Network Slicing
...
- **의사결정**:
  1. **슬라이스 1 (공장 제어망 - URLLC)**: 가장 중요한 로봇 제어 신호를 위해 무선 구간에서는 PRB(주파수 자원)를 독점 할당(Hard Slicing)하고, 공장 내부에 엣지 클라우드(MEC)와 UPF를 배치하여 트래픽이 외부 인터넷으로 나가지 않고 공장 내에서 바로 처리되도록 초저지연 경로를 설계한다.
  2. **슬라이스 2 (센서망 - mMTC)**: 센서 데이터 수집용 슬라이스는 통신사의 중앙 코어망을 공유하는 소프트 격리(Soft Slicing) 형태로 구축하여 인프라 비용을 절감한다.
...
- **안티패턴**: 하나의 슬라이스에 너무 많은 이질적 서비스를 몰아넣는 '과소 분할'이나, 반대로 서비스마다 슬라이스를 남발하여 오버헤드로 인해 스위칭 및 관리 코스트가 급증하는 '과다 분할' 모두 망 성능을 저하시킨다. 전체 망 자원 대비 적절한 수의 슬라이스 템플릿(보통 수십 개 이내) 운영이 필요하다.
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **SDN (Software Defined Networking)** | 트래픽 경로 제어 평면을 분리하여 슬라이스별로 동적인 라우팅 규칙을 내리는 두뇌 역할. |
| **NFV (Network Functions Virtualization)** | 고가의 통신 전용 장비들을 범용 서버 위의 가상머신(VNF)으로 띄워 슬라이스 생성 비용을 낮추는 기반 기술. |
| **MEC (Multi-access Edge Computing)** | 초저지연 슬라이스(URLLC)를 구현하기 위해 코어 기능을 사용자와 물리적으로 가장 가까운 기지국 단에 전진 배치하는 아키텍처. |
| **S-NSSAI (Single Network Slice Selection Assistance Information)** | 단말(스마트폰, IoT 등)이 자신이 어떤 슬라이스에 접속해야 하는지를 망에 알려주는 고유 식별자. |
| **O-RAN (Open RAN)** | 폐쇄적인 기지국 구간(RAN)의 인터페이스를 개방하여, 슬라이스별 세밀한 무선 자원 할당과 혁신을 가능하게 하는 구조. |
