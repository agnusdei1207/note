...
weight = 985
title = "985. X.509 인증서"
description = "X.509 인증서의 구조, 검증 원리 및 확장 필드(v3) 심층 분석"
date = 2024-05-24
[taxonomies]
tags = ["Network", "Security", "X.509", "Certificate", "PKI"]
categories = ["3_network"]
+++

# X.509 인증서 (X.509 Certificate)

> ⚠️ 이 문서는 PKI의 핵심 데이터 구조인 X.509 인증서의 내부 필드 구성, 파싱 메커니즘, 그리고 v3 확장을 통한 실무적 보안 통제 방안을 심층적으로 분석합니다.

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: X.509는 통신 주체의 식별 정보(DN, Distinguished Name)와 공개키(Public Key)를 바인딩하고, 이를 인증기관(CA)의 전자서명으로 무결성을 보증하는 ITU-T의 '디지털 인증서 표준 규격'이다.
> 2. **가치**: ASN.1 기반의 엄격한 데이터 구조를 통해 이기종 시스템(윈도우, 리눅스, 안드로이드 등) 간에 신뢰 정보를 상호 운용할 수 있게 하며, 전 세계 모든 HTTPS 통신의 신원 확인 기반으로 동작한다.
> 3. **융합**: X.509 v3의 확장 필드(Extensions) 기능은 단순한 신원 확인을 넘어, 용도 제한(Key Usage), 다중 도메인 지원(SAN), 폐지 상태 확인(CRL/OCSP) 등 현대 웹 보안에 필요한 메타데이터를 유연하게 결합하는 아키텍처를 제공한다.
