...
weight = 991
title = "991. ARP 스푸핑"
description = "로컬 네트워크(LAN)의 신뢰를 악용하여 통신 흐름을 가로채는 ARP Spoofing의 원리와 L2 계층 방어 전략"
date = 2024-05-24
[taxonomies]
tags = ["Network", "Security", "ARP", "L2"]
+++

# 991. ARP 스푸핑

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: ARP 스푸핑 (ARP Spoofing)은 로컬 네트워크(LAN) 내에서 공격자가 자신의 MAC (Media Access Control) 주소를 다른 정상 기기(주로 기본 게이트웨이)의 IP 주소와 매핑시키는 조작된 ARP 응답 패킷을 방송하여 통신 흐름을 가로채는 기만 공격이다.
> 2. **가치**: 이 공격을 통해 공격자는 정상 사용자 모르게 네트워크 패킷을 도청(Sniffing)하거나 변조하는 중간자 공격 (MitM, Man-in-the-Middle)을 완벽히 수행할 수 있으며, 암호화되지 않은 사내망에서 평문 비밀번호와 세션을 탈취하는 가장 치명적인 내부 위협 요소다.
> 3. **융합**: ARP (Address Resolution Protocol) 자체의 상태를 검증하지 않는(Stateless) 구조적 한계를 보완하기 위해, 최신 네트워크 스위치 장비는 포트 보안(Port Security)과 DHCP 스누핑 기반의 동적 ARP 검사 (DAI, Dynamic ARP Inspection) 기술을 융합하여 L2 계층의 무결성을 하드웨어적으로 강제하고 있다.
