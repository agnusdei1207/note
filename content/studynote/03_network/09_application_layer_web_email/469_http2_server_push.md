...
### 비교 2: Server Push vs WebSocket vs SSE
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **HTTP/2 Multiplexing** | 하나의 TCP 연결에서 여러 스트림을 쪼개어 보내는 기술로, 서버 푸시가 본문 데이터와 병렬로 전송될 수 있는 물리적 기반을 제공한다. |
| **HOL Blocking (Head-of-Line Blocking)** | HTTP/1.1의 직렬 요청 병목 현상으로, 서버 푸시와 멀티플렉싱이 해결하고자 했던 핵심 문제다. |
| **TCP Congestion Window** | 서버 푸시 시 초기에 너무 많은 데이터를 밀어넣으면 이 윈도우 한계에 부딪혀 정작 중요한 HTML 파싱이 늦어지는 부작용을 일으킨다. |
| **Preload / Prefetch** | 브라우저가 주도적으로 리소스를 당겨오는(Pull) 기술로, 서버가 밀어넣는(Push) 방식의 대안이자 경쟁 기술이다. |
| **HTTP 103 Early Hints** | 서버 푸시의 대역폭 낭비 문제를 해결하기 위해 고안된 차세대 표준으로, 서버는 메타데이터 힌트만 제공하고 요청 판단은 브라우저에 위임한다. |
