+++
weight = 314
title = "314. 강화 학습 MDP (Markov Decision Process) - 마르코프 결정 과정 환경"
date = "2026-04-21"
[extra]
...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: MDP (Markov Decision Process, 마르코프 결정 과정)는 강화 학습의 수학적 기반으로, 에이전트(Agent)가 환경(Environment)과 상호작용하는 문제를 **상태(State)·행동(Action)·보상(Reward)·전이 확률(Transition Probability)**의 4요소로 정형화하는 프레임워크다.
> 2. **가치**: "현재 상태만 알면 미래 결정에 충분하다"는 **마르코프 성질(Markov Property)**이 복잡한 시퀀스 의사결정 문제를 계산 가능한 최적화 문제로 변환하여, 게임 AI·로봇 제어·자율주행·자원 관리 등 광범위한 최적 제어 문제를 해결한다.
...
로봇이 미로를 탈출하는 법을 배운다고 가정하자. 모든 좌회전·우회전 경우의 수를 미리 프로그래밍하는 것은 불가능하다. 대신 로봇이 스스로 시행착오를 통해 "이 방향으로 가면 막히고, 저 방향은 출구가 나온다"를 학습하게 하면 어떨까? 이것이 **강화 학습(Reinforcement Learning)**이다.

MDP는 이 과정을 수학적으로 표현한다:
- **상태(State, S)**: 환경의 현재 상황 (로봇 위치)
- **행동(Action, A)**: 에이전트가 취할 수 있는 행동 (상·하·좌·우)
- **보상(Reward, R)**: 행동 후 받는 즉각 피드백 (+10: 출구, -1: 매 걸음)
- **전이 확률(P)**: 상태 s에서 행동 a 후 상태 s'로 전이될 확률
...
**벨만 방정식 (Bellman Equation)**: 가치 함수를 재귀적으로 표현하는 핵심 방정식
...
1. **MDP(강화 학습의 설계도)**는 로봇이 미로 탈출하는 법을 배울 때, **"어떤 상황(상태), 어떤 행동, 얼마나 좋은지(보상)"를 수학으로 정의**한 것이에요!
2. 로봇은 직접 이리저리 움직여보며 **보상이 많은 길을 기억**하고, 나중에 막힘 없이 탈출하는 전략(정책)을 스스로 찾아요.
3. 체스(알파고), 게임 AI, 자율주행, **ChatGPT의 RLHF**까지 모두 이 MDP 방식으로 학습해요!
