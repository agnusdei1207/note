+++
weight = 336
title = "336. 텐서 코어 (Tensor Core) - 혼합 정밀 연산 가속 하드웨어"
date = "2026-04-21"
[extra]
...
FP16 의 최솟값 약 6×10⁻⁵ 보다 작은 그래디언트는 **언더플로우 (Underflow)** 로 0이 된다. 해결책: Loss 에 2¹⁶ 를 곱해 그래디언트를 증폭한 뒤, FP32 변환 후 스케일 나눔.
...
- **cuBLAS (CUDA Basic Linear Algebra Subroutines)**: GEMM 을 텐서 코어로 자동 라우팅
- **cuDNN (CUDA Deep Neural Network library)**: 합성곱 알고리즘 선택 시 텐서 코어 우선 활용
...
텐서 코어는 현대 딥러닝 가속의 핵심 하드웨어 혁신이다. 기술사 시험에서는 WMMA 연산 구조, Mixed Precision 파이프라인, Loss Scaling 필요성, BF16 vs FP16 비교를 체계적으로 서술하면 고득점 가능하다.
...
### 👶 어린이를 위한 3줄 비유 설명
- 🔢 텐서 코어는 "수학 숙제를 한 문제씩 푸는 대신, 4×4 표 전체를 한 번에 푸는 초고속 수학 기계"예요.
- 💡 FP16 은 더 빠르지만 소수점이 덜 정확해서, 중요한 결과만 FP32 로 꼼꼼히 보관해요.
- 🚀 이 덕분에 같은 GPU 로 2~3배 빠르게, 메모리도 절반만 써서 AI 를 학습할 수 있어요!
