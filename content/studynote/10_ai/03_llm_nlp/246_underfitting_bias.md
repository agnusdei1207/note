+++
weight = 246
title = "246. 과소 적합 (Underfitting) - 편향 오류"
date = "2026-04-21"
[extra]
...
> 2. **가치**: 과소적합 진단은 모델 선택(Model Selection)과 특성 공학(Feature Engineering)의 방향성을 결정하는 핵심 신호이며, 머신러닝 프로젝트 초기 단계에서 반드시 점검해야 한다.
...
- **편향(Bias)**: 예측값의 기댓값과 실제 정답의 차이 — 모델의 체계적 오류
...
| **다항 특성(Polynomial Feature)** | 기존 특성의 거듭제곱 및 교차항 추가 | 비선형 경계 표현 가능 |
| **규제 완화** | λ(정규화 계수) 감소 | 가중치 자유도 증가 |
| **에폭 증가** | 학습 반복 횟수 늘림 | 수렴 기회 증가 |
| **다른 알고리즘 선택** | 선형→트리 기반→신경망 | 근본적 표현력 향상 |
...
### 3.3 특성 공학(Feature Engineering)으로 편향 감소
...
과소적합은 **곱셈을 배우기 전에 나눗셈 문제를 풀려는 것**과 같아요.  
모델이 너무 단순해서 문제의 핵심을 아예 못 배우는 상태예요.  
이럴 때는 더 많은 규칙(특성)을 배우거나 더 복잡한 방법(모델)으로 공부하면 해결돼요!
