+++
weight = 260
title = "260. 부스팅 (Boosting) - 오차 가중치 직렬 보완 / XGBoost"
date = "2026-04-21"
[extra]
...
배깅이 분산을 줄이는 데 집중한다면, 부스팅은 **편향(Bias)을 줄이는** 데 집중한다.
...
4. **AdaBoost**: 교육용, 이론 학습 목적

### 기술사 답안 포인트

- **"부스팅이 편향을 줄이는 원리"**: 잔차 반복 학습 → 각 트리가 이전 트리가 설명 못한 패턴 학습
- **"XGBoost가 빠른 이유"**: Column Block으로 데이터 정렬 캐시, 병렬 노드 분할
- **"부스팅 vs 배깅 선택 기준"**: 고편향 → Boosting, 고분산 → Bagging
...
1. **최고 수준의 예측 성능**: 정형 데이터(Tabular Data) 분야에서 딥러닝과 경쟁 또는 압도
...
기술사 시험에서 XGBoost는 **Gradient Boosting 원리 → XGBoost 개선점 → 하이퍼파라미터 조정 → 과적합 방지** 순서로 체계적으로 서술해야 고득점을 받는다.
...
| 학습률 (Learning Rate) | η, Shrinkage | 부스팅 과적합 제어 |
| 조기 종료 (Early Stopping) | 검증 손실, n_estimators | 과적합 방지 핵심 기법 |
