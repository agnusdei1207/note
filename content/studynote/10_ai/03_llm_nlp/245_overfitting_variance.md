+++
weight = 245
title = "245. 과대 적합 (Overfitting) - 분산 오류"
date = "2026-04-21"
[extra]
...
- **편향(Bias)**: 모델이 단순하여 실제 패턴을 포착하지 못하는 오차 → 과소적합
- **분산(Variance)**: 모델이 복잡하여 훈련 데이터의 잡음에 민감하게 반응하는 오차 → 과대적합
...
| 기법 | 원리 | 효과 |
|:---|:---|:---|
| **L1 규제(Lasso)** | 가중치 절댓값 합 패널티 추가 | 불필요한 특성 가중치 0으로 만들어 희소 모델 생성 |
| **L2 규제(Ridge)** | 가중치 제곱합 패널티 추가 | 모든 가중치를 작게 유지하여 분산 감소 |
| **드롭아웃(Dropout)** | 학습 시 뉴런을 확률적으로 비활성화 | 특정 뉴런에 의존하지 않는 강건한 표현 학습 |
| **조기 종료(Early Stopping)** | 검증 오차가 증가하면 학습 중단 | 최적 에폭(Epoch)에서 학습 종료 |
| **데이터 증강(Data Augmentation)** | 기존 데이터를 변환하여 다양성 추가 | 훈련 데이터 효과적 확장 |
| **앙상블(Ensemble)** | 여러 모델의 예측 결합 | 개별 모델의 과대적합 평균화 |
...
- **L1 (Lasso)**: `손실함수 + λΣ|w|` → 일부 가중치를 정확히 0으로 만들어 특성 선택 효과
- **ElasticNet**: L1 + L2 혼합
...
### 4.3 기술사 핵심 판단 포인트
- 과대적합 해결 시 **데이터 증가 vs 규제 강화** 중 어느 것이 더 효과적인지 상황별 판단
- 딥러닝에서는 **드롭아웃 + 배치 정규화(Batch Normalization)** 조합이 표준
- L1/L2 선택은 **희소성(Sparsity) 필요 여부**로 결정
...
과대적합은 **답안지를 달달 외웠는데 문제가 조금만 바뀌어도 틀리는 것**이에요.  
모델이 훈련 데이터를 너무 열심히 암기해서 새로운 문제는 못 푸는 상태예요.  
이럴 때는 드롭아웃(공부할 때 일부러 헷갈리게 연습)이나 규제(너무 세세하게 외우지 못하게 제한)로 고쳐줄 수 있어요!
