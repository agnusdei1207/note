+++
weight = 372
title = "372. 벨만 방정식 (Bellman Equation) - Q-러닝 상태 가치 수식"
date = "2026-04-21"
[extra]
...
벨만(Bellman, 1957)은 이 문제를 재귀적 최적성 원칙(Principle of Optimality)으로 해결했다. **현재 상태의 최적 가치는 즉각 보상과 다음 상태의 최적 가치의 합**이라는 단순한 원리로 복잡한 순차적 결정 문제를 분해한다.
...
**탐험-착취 균형(Exploration-Exploitation Tradeoff)**:
...
DQN의 핵심 기법:
1. **경험 재현(Experience Replay)**: 과거 경험 (s,a,R,s')을 버퍼에 저장 후 랜덤 샘플링 → 데이터 상관성(Correlation) 제거
2. **타겟 네트워크(Target Network)**: Q-업데이트 타겟 계산에 별도 네트워크 사용 → 학습 안정성 향상
