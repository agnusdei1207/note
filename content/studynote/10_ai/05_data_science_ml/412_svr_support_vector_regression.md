...
| **Kernel** | 비선형 투영 | 복잡한 패턴 학습 |
| **Support Vectors** | 튜브 경계/밖에 있는 샘플 | 모델을 실제로 정의하는 핵심 점 |
...
기술사 관점에서는 "SVR은 좋은 도구지만, 데이터가 크면 무거워질 수 있다"는 점을 함께 말해야 한다. 작은~중간 규모 데이터와 비선형 회귀에서 특히 유리하다.
...
결론적으로 SVR은 **'허용 오차가 있는 회귀'**이므로, 우리는 데이터 크기·스케일·커널 선택을 함께 고려해야 한다.
