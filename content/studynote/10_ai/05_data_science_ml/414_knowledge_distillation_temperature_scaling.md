...
| **Temperature (T)** | 확률을 부드럽게 함 | 클래스 간 관계 노출 |
| **Soft Target** | 교사 분포 | 모호성/유사성 학습 |
...
기술사 관점에서는 "지식 증류는 단순 압축이 아니라, 출력 분포의 구조를 전달하는 교육 과정"이라고 설명하면 좋다. 분류 경계뿐 아니라 클래스 간 관계까지 학습시키는 것이 핵심이다.
...
결론적으로 지식 증류는 **'큰 모델의 지식을 작은 모델로 압축하는 기술'**이므로, 우리는 온도와 손실 균형을 설계해 정보 손실을 최소화해야 한다.
