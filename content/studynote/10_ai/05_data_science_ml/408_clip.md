...
**필요성**:
- **데이터 효율성**: 수작업 라벨링 대신 인터넷의 풍부한 이미지-캡션 데이터를 활용
...
| 구성 요소 | 설명 | 특징 |
|:---|:---|:---|
| **Image Encoder** | 이미지를 고차원 벡터로 변환 | ViT (Vision Transformer) 또는 ResNet 사용 |
| **Text Encoder** | 텍스트를 고차원 벡터로 변환 | Transformer 기반 모델 사용 |
| **Joint Embedding** | 이미지와 텍스트 벡터가 만나는 공간 | 의미가 같은 쌍은 가깝게, 다른 쌍은 멀게 배치 |
| **Contrastive Loss** | 실제 쌍의 유사도는 높이고 나머지는 낮춤 | 행렬 대각선 성분의 확률을 최대화 |
...
CLIP은 408번의 **대조 학습(Contrastive Learning)** 기법을 활용하며, 320번의 **디퓨전 모델(Diffusion)**에서 사용자가 입력한 텍스트에 맞는 이미지를 생성하도록 가이드하는 '나침반' 역할을 한다.
...
2. **Domain Adaptation**: 일반적인 인터넷 데이터로 학습되었으므로, 의료나 정밀 제조 등 특수 도메인에서는 추가적인 미세 조정이 필요할 수 있다.
3. **Retrieval 활용**: 대규모 이미지 DB에서 특정 텍스트 설명과 일치하는 이미지를 초고속으로 찾는 시맨틱 검색(Semantic Search)에 즉시 적용 가능하다.

### 기술사 판단 포인트
- CLIP은 **'인간의 언어적 이해력이 시각적 인지력을 가이드한다'**는 철학을 증명했다. 이는 AI가 단일 감각(Unimodal)을 넘어 인간처럼 오감을 통합하는 **'일반 인공지능(AGI)'**으로 진화하는 핵심 징검다리임을 강조해야 한다.
