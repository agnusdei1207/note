...
1. **셀 상태(Cell State) C_t**: 장기 기억(Long-term Memory)을 담당. 게이트에 의해 선택적으로 업데이트.
2. **은닉 상태(Hidden State) h_t**: 단기 출력(Short-term Output)을 담당. 셀 상태를 기반으로 계산.
...
📢 **섹션 요약 비유**: 기술사 시험에서 LSTM 수식을 쓸 때 핵심은 "덧셈(⊕)"이다. C_t = f_t⊙C_{t-1} + i_t⊙C̃_t에서 덧셈 경로가 그래디언트 고속도로를 만든다는 것을 반드시 강조해야 한다.
