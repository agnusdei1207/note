+++
weight = 380
title = "380. 기울기 소실/폭발 (Vanishing/Exploding Gradient) - Kaiming He 초기화"
date = "2026-04-21"
[extra]
...
ReLU의 경우 음수 입력이 모두 0이므로 **활성화 분산이 입력 분산의 절반**이 된다.
...
- **잔차 연결 (Residual Connection, ResNet)**: F(x) + x → 기울기 고속도로
- **배치 정규화 (Batch Normalization)**: 중간 활성화 정규화로 안정화
- **기울기 클리핑 (Gradient Clipping)**: 폭발 방지, RNN에서 주로 사용
- **LSTM/GRU**: 게이트 구조로 장기 의존성 보존
...
기술사 판단: 학습 초반 손실이 전혀 감소하지 않으면 → 기울기 소실 의심 → 초기화 방법, 활성화 함수, 네트워크 깊이 점검.
