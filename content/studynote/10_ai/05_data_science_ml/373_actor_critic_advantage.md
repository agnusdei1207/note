+++
weight = 373
title = "373. Actor-Critic (A2C) 와 Advantage - 행동자-비평자 강화학습"
date = "2026-04-21"
[extra]
...
이 방법은 **고분산(High Variance)** 문제를 가진다. 같은 (s,a) 쌍이라도 에피소드 전개에 따라 G_t가 크게 달라지므로, 그래디언트가 불안정하고 수렴이 느리다.

해결책은 두 가지 방향이다:
1. **베이스라인(Baseline)** 차감: G_t 대신 G_t - b(s_t) 사용 (분산 감소, 기댓값 불변)
...
**PPO (Proximal Policy Optimization)** 핵심:
...
**엔트로피 보너스(Entropy Bonus)**: Actor 손실에 H(π)를 더해 탐험 촉진:
```
