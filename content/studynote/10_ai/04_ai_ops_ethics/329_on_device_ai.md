+++
weight = 329
title = "329. 온디바이스 AI (On-Device AI) - 엣지 추론 NPU"
date = "2026-04-21"
[extra]
...
자율주행의 경우 더 명확하다. 브레이크 여부를 클라우드에 물어보고 100ms 후 답을 받으면 이미 사고다. **엣지에서 1ms 내 결정**이 생명을 구한다. 보안 카메라의 얼굴 인식도 영상을 클라우드로 보내면 GDPR 위반 — 디바이스에서 처리하면 개인정보 문제가 없다.
...
5. **모델 업데이트 전략**: OTA(Over-The-Air) 업데이트 파이프라인 설계 (연합 학습과 연계 가능)
...
1. **온디바이스 AI**는 AI가 인터넷 서버가 아닌 **스마트폰이나 자동차 안에서** 바로 동작하는 거예요!
2. 덕분에 **인터넷 없어도 AI가 작동**하고, 개인 정보가 서버로 나가지 않아 **프라이버시도 보호**돼요.
3. **Apple Intelligence, Galaxy AI**처럼 요즘 폰에 탑재되는 AI 기능이 모두 **NPU 칩 위에서 돌아가는** 온디바이스 AI예요!
