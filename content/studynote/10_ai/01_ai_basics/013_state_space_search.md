...
| **초기 상태 (Initial State)** | 탐색을 시작하는 출발 노드 | 게임이 섞인 직후의 타일 배치 |
...
|:---|:---|:---|:---|
| **도메인 지식** | 없음 (오직 목표 판별만 가능) | 있음 (목표까지의 거리 추정치 $h(n)$) | 오직 현재 상태의 이웃만 평가 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| **BFS (Breadth-First Search)** | 최단 경로를 보장하는 너비 우선 맹목 탐색, 메모리 집약적 |
| **A* 알고리즘 (A-star)** | 휴리스틱 함수로 유망 노드 우선 탐색, 최적성 보장 |
| **MCTS (Monte Carlo Tree Search)** | 롤아웃 시뮬레이션으로 거대 상태 공간을 부분 탐색하는 알파고 핵심 기술 |
