...
date = "2024-05-20"
[taxonomies]
tags = ["AI", "KnowledgeRepresentation", "의미망", "온톨로지", "규칙기반"]
categories = ["10_ai"]
+++
...
가장 현대적으로 활용되는 **의미망 (Semantic Network) 기반 온톨로지(Ontology)**의 내부 동작 메커니즘은 다음과 같다.

① **노드 정의 (Entity 정의)**: 현실 세계의 객체(예: '사람', '자동차', '아우디')를 식별자로 노드화한다.
② **관계 매핑 (Relation 연결)**: 객체 간의 종속/소유 관계를 트리플(Triple: 주어-서술어-목적어) 형태로 연결한다. (예: 아우디 --is a--> 자동차)
③ **제약조건 할당**: 특정 속성에 허용되는 값의 타입과 범위를 부여한다.
④ **추론 전파 (Propagation)**: 부모 노드의 속성이 자식 노드로 상속(Inheritance)되어, 명시되지 않은 새로운 사실을 도출한다.
⑤ **그래프 쿼리 (SPARQL)**: 완성된 의미망을 순회하며 다중 홉(Multi-hop) 논리 질의에 응답한다.
...
최근 딥러닝과 기호주의 AI의 융합 관점에서 보면, **임베딩(Embedding) 벡터와 지식 표현의 융합**이 가장 큰 화두다.
...
실무에서 지식 표현 기법을 도입할 때 가장 큰 장벽은 **'지식 획득의 병목(Knowledge Acquisition Bottleneck)'**이다. 인간 전문가의 머릿속에 있는 지식을 추출하여 기계어로 변환하는 과정 자체가 막대한 시간과 비용을 소모한다.

**실무 시나리오 1: 엔터프라이즈 사내 문서 검색(RAG) 고도화**
...
**실무 시나리오 2: 금융권 사기 탐지(FDS) 시스템 구축**
...
|:---|:---|:---|
| **논리 추론 에러율** | 블랙박스 모델 의존으로 간헐적 치명적 오류 (환각) | 규칙 및 상속 기반 논리 검증으로 도메인 오류 99% 억제 |
| **데이터 재사용성** | 개별 모델마다 데이터 파싱 필요 (Silo) | 표준 온톨로지(OWL) 기반으로 타 시스템과 지식 교환 자유로움 |
| **감사(Audit) 대응** | 판단 근거 추적 불가 | 적용된 규칙과 거쳐간 노드 경로가 완벽히 로깅됨 |
