...
전향 추론 엔진은 내부적으로 작업 메모리(Working Memory)의 상태 변화를 감지하여 규칙을 실행하는 무한 루프 아키텍처로 구성된다. 이를 **Match-Resolve-Act 사이클**이라고 부른다.
...
전향 추론의 심층 동작 원리는 단방향이 아니라 **상태의 순환 갱신**이다.
...
**실무 시나리오 1: 스마트 팩토리의 실시간 설비 이상 탐지 (CEP)**
...
**실무 시나리오 2: 무분별한 Fact 증식으로 인한 Out Of Memory 장애**
