...
```text
  ┌───────────────────────────────────────────────────────────────┐
  │         데이터베이스 아키텍처 패러다임: 단일 CRUD vs CQRS 분리 마술      │
  ├───────────────────────────────────────────────────────────────┤
  │                                                               │
  │  [A. 레거시 CRUD 아키텍처 (Lock과 병목의 지옥 💥)]                   │
  │   [ 유저 A: 결제 (Write) ] ──▶ [ 단일 웹서버 ] ──▶ 🗄️ [ 단일 DB ]   │
  │                               (병목 터짐)       (Lock 경합 발생) │
  │   [ 유저 B: 조회 (Read)  ] ──▶               ──▶ 🗄️            │
  │   ★ 참사: 1명이 글 쓸 때, 1만 명의 조회 유저가 문 앞에서 대기해야 함.           │
  │           조회 성능 높이려고 캐시 달고 인덱스 달면, 이번엔 글 쓰기가 느려짐.     │
  │                                                               │
  │  [B. CQRS 아키텍처 (극단적 이원화 🚀)]                               │
  │                                                               │
  │   [ 유저 A: 결제 (Write) ] ──▶ [ Command 서버 ] ──▶ 🗄️ [ Write DB ] │
  │    (주문 1건 10% 트래픽)          (무결성 빡세게 검사)    (관계형 RDBMS)  │
  │                                                        │      │
  │                     (비동기 동기화: Message Queue/Kafka) │      │
  │                                                        ▼      │
  │   [ 유저 B: 조회 (Read)  ] ──▶ [ Query 서버 ] ◀──── 🗂️ [ Read DB ]  │
  │    (조회 1만 건 90% 트래픽)       (그냥 던져줌 초고속)     (NoSQL/Redis)  │
  │                                                               │
  │   ★ 기적: A가 결제하든 말든 B는 다른 컴퓨터(Read DB)에서 맘껏 조회를 한다!   │
  │           Write DB에 글이 써지면, 뒷단 로봇(카프카)이 몰래 Read DB로 데이터를│
  │           화면 모양에 딱 맞춰 예쁘게 가공해서 0.1초 만에 쏴줌(최종 일관성).  │
  └───────────────────────────────────────────────────────────────┘
```
