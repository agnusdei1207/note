...
| 개념 | 연결 포인트 |
|:---|:---|
| **버블 정렬 (Bubble Sort)** | 동일 O(N²) 안정 정렬이나 교환 횟수가 더 많아 실제 성능은 삽입 정렬이 우세 |
| **셸 정렬 (Shell Sort)** | 삽입 정렬을 간격(gap) 단위로 반복·수렴시켜 평균 O(N^1.5)로 개선한 변형 알고리즘 |
| **Timsort** | Python·Java 표준 라이브러리 정렬로, 소규모 런(Run)에 삽입 정렬을 적용하는 하이브리드 알고리즘 |
