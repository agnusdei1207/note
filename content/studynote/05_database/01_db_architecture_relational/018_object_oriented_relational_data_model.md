...
[taxonomies]
categories = ["Database"]
tags = ["OODBMS", "ORDBMS", "Object-Oriented", "Data Model", "Impedance Mismatch"]
+++
...
|:---|:---|:---|:---|:---|
| **OID (객체 식별자)** | 객체의 절대적 고유성 보장 | 데이터 값 변경과 무관하게 시스템이 자동 할당 | PK(기본키)는 데이터 값에 의존함 | 사람의 지문(변하지 않음) |
...
| **상속 (Inheritance)** | 객체 구조의 재사용 및 확장 | 슈퍼타입의 속성과 메서드를 서브타입이 물려받음 | 테이블 조인이나 슈퍼/서브 패턴 필요 | 부모의 유전자 물려받기 |
...
| **데이터 언어** | SQL (선언적, 강력한 최적화) | OQL, 포인터 네비게이션 (절차적 성격) | SQL3 / SQL:1999 (SQL 기반 객체 확장) | 기존 인력과 생태계 호환성 |
| **복잡 데이터 처리** | 조인 오버헤드 큼 | 포인터 기반으로 복잡한 구조 초고속 처리 | 복잡 구조 지원, 조인 최소화 | 멀티미디어/공간 데이터 적합성 |
| **무결성 및 보안** | 최고 수준 (ACID 완벽) | 상대적으로 약함 (애플리케이션 의존) | 최고 수준 (RDB 엔진 그대로 사용) | 금융 등 엔터프라이즈 신뢰성 |
...
| **도메인 확장** | 비정형 특수 데이터(GIS, XML, 시계열, 유전체)를 네이티브 수준의 인덱스로 처리 |
| **성능 튜닝** | 다치 속성(Array) 활용으로 무거운 다중 테이블 조인 비용(CPU, Memory) 극단적 감소 |
