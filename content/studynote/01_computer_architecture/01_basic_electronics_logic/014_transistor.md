...
| **전력 소모** | 통전 시 지속적 전력 소모 | 통전 시 지속적 전력 소모 | **대기 전력 소모가 사실상 0 (Zero)** |
...
2. 초미세 노드 진입 시 전류 통제력을 회복하기 위해 3면을 덮는 **FinFET**이나 4면을 모두 덮는 **GAA (Gate-All-Around)** 3D 아키텍처 공정이 수율을 달성했는가?
...
| 개념 | 연결 포인트 |
|:---|:---|
| **CMOS (Complementary MOS)** | PMOS와 NMOS를 상보적으로 결합해 대기 전력을 0으로 수렴시킨 회로 구조 |
| **FinFET & GAA** | 짧아진 채널의 누설 전류를 막기 위해 게이트 접촉면을 3D 입체로 확대한 차세대 칩 아키텍처 |
| **문턱 전압 (Threshold Voltage, Vth)** | 트랜지스터의 채널을 열기 위해 가해져야 하는 최소 전압 (낮을수록 빠르지만 누수가 큼) |
