...
평면 구조에서는 성능을 높이려면 채널의 폭을 넓히면 됐지만, FinFET에서는 이미 깎인 핀의 높이를 맘대로 키울 수 없으므로(부러짐 위험), 핀을 2개, 3개 옆으로 나란히 늘어놓는 **Multi-Fin 방식**으로 구동 전류를 늘린다. 고속 코어에는 핀을 4개씩 깔고, 저전력 코어에는 1~2개만 까는 식이다.
