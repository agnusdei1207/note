...
| 단계 | 활동 | 산출물 |
|:---|:---|:---|
| **1. 위험 식별** | 사업 환경, 기술 스택, 팀 역량 분석 | 위험 목록(Risk Register) |
| **2. 위험 평가** | 발생 가능성 × 영향도 = 위험도 산정 | 위험 매트릭스 |
| **3. 감리 계획** | 위험도 기반 감리 점검 항목·시간 배분 | 감리 계획서 |
| **4. 감리 수행** | 고위험 영역 심층 검토 | 감리 조서 |
...
| **보안·개인정보** | 법적 의무 불이행 → 과징금 |
| **성능 요구사항** | 과소 테스트 → 운영 환경 병목 |
| **데이터 전환** | 오류 → 서비스 오픈 후 데이터 정합성 문제 |
...
1. **위험 식별**: 레거시 대용량 데이터 전환(500GB), 36개 외부 시스템 연동, 2회 요구사항 변경 이력.
