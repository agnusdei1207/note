...
## 핵심 인사이트 (3줄 요약)
- **비트 연산을 이용한 구간 합 최적화**: 2진수의 마지막 1비트(`i & -i`)가 관리하는 구간의 길이를 결정하는 원리를 이용하여 구간 합 쿼리와 값 업데이트를 모두 $O(\log n)$에 처리함.
...
```
- **핵심 비트 연산:** `i & -i`는 $i$를 2진수로 나타냈을 때 가장 오른쪽에 있는 1의 비트값(LSB)을 반환함 (예: $6(0110) \rightarrow 2(0010)$).
