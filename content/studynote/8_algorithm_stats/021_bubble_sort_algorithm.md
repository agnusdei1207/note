...
- **필요성**: 그럼에도 불구하고 모든 컴퓨터 과학 커리큘럼에서 거품 정렬을 가르치는 이유는, 데이터 구조 내에서 포인터 이동, 인덱스 제어, 임시 변수(Temp)를 이용한 메모리 값 스왑(Swap) 등 **제어문(Control Flow)의 근본 원리**를 가장 명확하게 보여주는 '투명한 뼈대'이기 때문입니다.
