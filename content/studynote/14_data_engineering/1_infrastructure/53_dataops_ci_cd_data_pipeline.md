...
> ⚠️ 이 문서는 소프트웨어 개발의 DevOps 철학을 데이터 엔지니어링에 도입하여, 수집부터 분석, 적재(ETL)로 이어지는 **데이터 파이프라인 전체의 코드 형상 관리(버저닝), 품질 테스트 자동화, CI/CD를 통해 고품질의 신뢰할 수 있는 데이터를 분석가에게 신속하게 배달하는 'DataOps' 혁신 방법론**을 다룹니다.
...
2. **데이터 단위 테스트 (Automated Data Testing)**:
   - 코드를 배포하기 전에 CI 파이프라인이 돌면서 **데이터 무결성 검사**를 수행한다.
