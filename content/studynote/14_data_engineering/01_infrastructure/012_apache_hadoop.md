...
date = "2023-10-24"
[taxonomies]
tags = ["Hadoop", "Apache Hadoop", "HDFS", "MapReduce", "Big Data", "Data Engineering"]
...
|:---|:---|:---|:---|:---|
| **HDFS (저장)** | 분산 데이터 저장 | 거대 파일을 128MB 블록으로 쪼개 3벌씩 여러 데이터노드에 분산 복제 보관 | RPC, TCP | 거대 분산 창고 |
| **MapReduce (연산)** | 분산 배치(Batch) 처리 | Map(필터링/매핑) → Shuffle(정렬/그룹화) → Reduce(집계) 과정을 디스크 기반으로 수행 | Java API | 수만 명의 데이터 분석가 |
| **YARN (자원 관리)** | 클러스터 자원 스케줄링 | 전체 노드의 CPU/Memory 현황을 파악하여 각 애플리케이션(Spark, MapReduce)에 자원 컨테이너 할당 | RPC | 인력 사무소 소장 |
| **NameNode (마스터)** | HDFS 메타데이터 중앙 관리 | 블록이 어느 데이터노드에 있는지 디렉터리 구조와 매핑 테이블을 RAM에서 관리 | HDFS Protocol | 창고 장부(목록) 관리자 |
| **DataNode (워커)** | 실제 데이터 블록 입출력 | 클라이언트의 쓰기/읽기 요청을 처리하고, 주기적으로 NameNode에 Heartbeat 보고 | TCP/IP | 실제 물건을 쌓는 창고지기 |
...
|:---|:---|:---|:---|
| **데이터 처리 모델** | 디스크(Disk) 기반, 단계마다 HDFS에 임시 결과 저장 | 인메모리(In-Memory) 기반, RDD를 통해 메모리에서 파이프라인 처리 | 처리 속도 요구치 |
| **적합한 워크로드** | 대규모 야간 배치(Batch) 처리, ETL 초기 정제 | 머신러닝(ML) 반복 훈련, 실시간 스트리밍, 대화형 쿼리 | 반응성(Latency) |
| **장애 복구 방식** | 디스크에 기록된 체크포인트에서 재시작 | RDD 리니지(Lineage, 연산 계보)를 통해 메모리 유실 시 즉시 재연산 | 장애 시 지연 시간 허용 범위 |
