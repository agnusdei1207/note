...
date = "2025-01-01"
[taxonomies]
tags = ["MapReduce", "Hadoop", "Distributed Computing", "Big Data", "Shuffle"]
...
| **Shuffle & Sort** | 재배치 및 그룹화 | **(가장 중요)** 네트워크를 통해 동일한 Key를 가진 데이터를 모으고 정렬하여 특정 리듀서로 보냅니다. | HTTP Fetch / Disk I/O | 택배 분류 허브 |
...
| **반복 알고리즘** | **치명적 병목** (매 반복마다 파일 읽기/쓰기) | **압도적 우위** (메모리에서 즉각 반복) | 머신러닝, 그래프 분석 적합성 |
| **장애 복구** | 즉각적인 태스크 재실행 (디스크 백업 안전) | 리니지(Lineage) 그래프 재연산 (약간의 복잡성) | 안정성 vs 속도 트레이드오프 |
