...
date = "2024-05-24"
[taxonomies]
tags = ["FaaS", "Serverless", "AWS Lambda", "Cloud Architecture", "Cold Start"]
...
| **API Gateway** | 클라이언트 접점 제공 | REST 엔드포인트 노출, 인증/인가, Throttling 후 페이로드 전달 | OAuth, JWT | 접수처 데스크 |
| **FaaS Controller** | 자원 오케스트레이션 | 가용 워커 노드를 탐색하고, 필요 시 새로운 인스턴스 프로비저닝 지시 | Kube-API | 중앙 관제 센터 |
| **MicroVM / Sandbox**| 안전한 격리 실행 환경 | Firecracker 등을 이용해 밀리초 단위로 초경량 가상머신 부팅 (커널 격리) | Firecracker | 무균 수술실 |
| **Stateless Runtime**| 비즈니스 로직 실행 | Node.js, Python 런타임을 로드해 코드 실행. 상태는 외부 저장소 의존 | V8 엔진 | 수술 집도의 |

이 도식은 사용자의 API 요청부터 FaaS 백엔드 엔진이 어떻게 마이크로VM을 할당하고 반환하는지를 보여주는 계층 구조도이다.
```text
┌─────────────────────────────────────────────────────────────┐
│ 1. Client Request (HTTP / S3 Event / Cron)                  │
└───────┬──────────────────────────────────────────────┬──────┘
        │ (Payload)                                    │ (Response)
┌───────▼──────────────────────────────────────────────┴──────┐
│ 2. API Gateway / Event Router (Auth, Rate Limit)            │
└───────┬──────────────────────────────────────────────┬──────┘
        │ (Invoke Function Event)                      │
┌───────▼──────────────────────────────────────────────┴──────┐
│ 3. FaaS Controller (Scale-out Manager)                      │
│   [ Idle Worker Pool ]   ==>   [ Cold Worker Init ]         │
└───────┬──────────────────────────────────────────────┬──────┘
        │ (Dispatch)                                   │
┌───────▼──────────────────────────────────────────────┴──────┐
│ 4. Worker Node (Physical Server)                            │
│ ┌────────────────┐ ┌────────────────┐ ┌────────────────┐    │
│ │ MicroVM (Warm) │ │ MicroVM (Cold) │ │ MicroVM (Term) │    │
│ │ - Node Runtime │ │ - Boot OS/Code │ │ - GC / Clean   │    │
│ │ - Execute()    │ │ - Download Pkg │ │                │    │
│ └────────────────┘ └────────────────┘ └────────────────┘    │
└───────┬──────────────────────────────────────────────┬──────┘
        │ (State Read/Write - Mandatory!)              │
┌───────▼──────────────────────────────────────────────┴──────┐
│ 5. External Backend Services (DynamoDB, S3, RDS)            │
└─────────────────────────────────────────────────────────────┘
```
...
| **스케일링 속도** | 수 분 (Minutes) | 수 초 (Seconds) | 밀리초 (Milliseconds) | 피크 트래픽 대응력 |
| **과금 기준** | VM이 켜진 시간 | 클러스터 할당 자원 풀 | 함수 호출 횟수 + 실행 시간 | 유휴 시간 비율 |
| **상태(State) 관리**| 자체 디스크 보존 | PV/PVC 활용 보존 가능 | 완전 무상태 (Stateless) | 외부 데이터베이스 의존도 |
...
|:---|:---|:---|:---|
| **비용 (FinOps)** | 월 100만 원 (상시 가동) | 월 5만 원 (가변) | 간헐적 트래픽 시스템 한정 최대 95% 절감 |
| **운영 공수 (Toil)**| 주 20시간 (OS/패치 관리) | 주 2시간 (코드 관리) | SRE 엔지니어의 로드 80% 이상 감소 |
| **스케일 아웃** | 분 단위 대기 (VM 부팅) | 수십 밀리초 반응 | 스파이크 트래픽에 대한 무중단 대응력 확보 |
