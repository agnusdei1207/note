...
> ⚠️ 이 문서는 기업이 특정 클라우드 서비스 제공자(CSP, 예: AWS, Azure, GCP)의 독자적이고 편리한 기술(예: 서버리스, 특화 DB)을 깊숙이 채택하여 쓰다가, **나중에 요금이 인상되거나 서비스 불만이 생겨도 다른 클라우드 벤더로 이사(Migration) 가기 위해 지불해야 하는 전환 비용(Switching Cost)이 너무 막대해져 발이 묶여버리는 함정**을 다룹니다.
