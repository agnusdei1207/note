...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 데이터 마트(Data Mart)는 전사 DW에서 특정 부서(영업·재무·마케팅)가 필요로 하는 데이터만 추출·요약한 **부서 전용 소규모 분석 저장소**다.
> 2. **가치**: 전사 DW의 방대한 테이블을 모두 조회하지 않고 **부서 관점에 최적화된 구조**로 미리 집계·재가공하여 쿼리 성능과 사용 편의성을 극대화한다.
> 3. **판단 포인트**: 독립형(Bottom-up) vs 종속형(Top-down)의 구조 선택이 핵심이며, 과도한 독립 마트는 **데이터 사일로**를 유발하므로 거버넌스 통제가 필요하다.
...
데이터 마트는 이 문제를 해결하기 위해 **부서별 관심 영역(Subject Area)**에 특화된 별도 저장소를 구성한다.
...
| 효과 | 내용 |
|:---|:---|
| **쿼리 성능 향상** | 전사 DW 전체 스캔 대신 부서 특화 데이터만 스캔 |
| **사용자 경험** | 분석가가 이해하기 쉬운 비즈니스 도메인 스키마 |
| **DW 부하 감소** | 부서별 쿼리를 마트에서 소화하여 DW 리소스 보호 |
...
| 한계 | 내용 |
|:---|:---|
| **데이터 사일로 위험** | 독립형 마트 다수 운영 시 부서 간 데이터 불일치 |
| **유지보수 비용** | 마트 수가 늘수록 ETL 파이프라인 관리 복잡 |
| **중복 저장** | DW와 마트에 유사 데이터 이중 저장 비용 |
| **스키마 변경 연쇄** | DW 스키마 변경 시 연결된 모든 마트 수정 필요 |
