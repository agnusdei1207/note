...
**도입 체크리스트 및 안티패턴**:
1. **사일로화된 도구 도입 (Anti-pattern)**: 각 부서가 편하다는 이유로 한쪽은 Spark, 한쪽은 다른 ETL 툴을 무분별하게 도입하면 데이터 거버넌스가 붕괴된다. 반드시 단일화된 데이터 카탈로그(예: Apache Atlas)로 메타데이터를 통제해야 한다.
...
3. **네트워크 Egress 비용**: 클라우드 간 또는 리전 간 데이터를 이동시킬 때 발생하는 전송 비용을 간과하여 "데이터 이동 배보다 배꼽이 큰" 상황이 없는지 검증해야 한다.
...
| **분석·시각화 (Analysis & Visualization)** | Hive, Superset, Kibana로 의사결정을 돕는 활용 계층 |
