...
**📢 섹션 요약 비유**: k-익명성은 **군중 속에 숨기**와 같다. 최소 k명이 같은 특징을 공유하면 누구인지 특정할 수 없다. l-다양성은 그 군중 내에 다양한 비밀(민감정보)이 섞여 있어야 한다는 추가 조건이다.
...
**📢 섹션 요약 비유**: 비식별화는 **예방 주사**와 같다. 완벽한 보호(100% 익명화)를 원하면 데이터가 쓸모없어지고, 보호를 안 하면 프라이버시가 침해된다. 적절한 수준의 보호(필요한 만큼의 백신)를 설계하는 것이 핵심이다.
...
- 비식별화는 **이름표를 가리는 것**이에요: 이름(직접 식별자)만 가리면 충분할 것 같지만, 나이+키+사는 동네 조합(준식별자)으로도 누구인지 알아낼 수 있어요.
