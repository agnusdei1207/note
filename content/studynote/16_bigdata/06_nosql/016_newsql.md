...
|:---:|:---:|:---:|:---|
| **CockroachDB** | PostgreSQL | Raft | 지역 파티셔닝, 서바이벌 목표 |
| **TiDB** | MySQL | Raft (TiKV) | HTAP(TiFlash), 클라우드 네이티브 |
...
### 결론
NewSQL은 "SQL 또는 확장성" 이분법을 극복한 현대 OLTP의 새 표준이다. 특히 글로벌 서비스와 데이터 주권 규정이 중요한 엔터프라이즈 환경에서 RDBMS 마이그레이션 경로로 주목받는다. 기술사 시험에서는 **Raft 합의 알고리즘 원리**, **HTAP(TiDB TiFlash)의 행/컬럼 이중 저장 구조**, **지역 파티셔닝과 데이터 주권**, **NewSQL vs RDBMS vs NoSQL 포지셔닝**이 핵심 논점이다.
