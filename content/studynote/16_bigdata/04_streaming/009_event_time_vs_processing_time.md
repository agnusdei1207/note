...
- **판단 포인트**: 이벤트 시간 처리는 Watermark (워터마크)를 통해 "얼마나 늦은 이벤트까지 기다릴 것인가"를 결정해야 하며, 이는 **정확성(Accuracy)과 지연(Latency)의 트레이드오프**로 워터마크가 클수록 정확하지만 늦게 결과가 나온다.
...
스트리밍 처리에서 "시간"은 단순한 타임스탬프가 아니라 **집계 결과의 정확성을 결정하는 핵심 기준**이다.
