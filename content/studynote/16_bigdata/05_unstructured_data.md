...
- **비정형 데이터의 RDBMS 강제 주입**: 애플리케이션 로그 텍스트를 파싱하지 않고 RDBMS의 VARCHAR/CLOB 컬럼에 무작정 때려 넣는 경우. 이후 특정 에러 로그를 찾기 위해 `LIKE '%error%'` 쿼리를 실행하면 데이터베이스 전체의 테이블 풀 스캔(Full Scan)이 발생하여 서비스가 마비된다.
- **Dynamic Mapping 맹신**: Elasticsearch를 사용할 때 인덱스 매핑을 자동으로 맡겨두면, 센서 데이터의 '100'(숫자)을 정수로 인식하다가 어느 순간 '"100"'(문자열)이 들어올 때 인덱싱 충돌이 발생해 그 이후의 모든 데이터가 적재되지 않는 조용한 장애가 발생한다.
