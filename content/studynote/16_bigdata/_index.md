...
- `[데이터 엔지니어링 파이프라인]`: 빅데이터 도구들을 엮어 데이터의 추출, 변환, 적재(ETL)를 오케스트레이션하는 설계론.
- `[데이터베이스와 트랜잭션]`: NoSQL과 NewSQL이 극복하고자 하는 전통적인 RDBMS의 한계와 ACID 원리.
- `[인공지능과 머신러닝]`: 정제된 빅데이터를 주입받아 스스로 패턴을 찾아내는 현대 IT의 최종 소비자.
- `[클라우드 분산 아키텍처]`: 수천 대의 노드를 자유롭게 스케일링하며 빅데이터의 연산력을 뒷받침하는 인프라스트럭처.
- `[알고리즘과 자료구조 (Hash/Tree)]`: 분산 노드 간의 셔플(Shuffle)을 최소화하고 검색(Index) 속도를 $O(1)$로 압살하는 근본 수학 논리.
