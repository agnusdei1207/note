...
|:---|:---|:---|:---|:---|
| **데이터 카탈로그** | 데이터의 위치와 출처 검색 | 메타데이터 수집, 데이터 리니지(Lineage) 추적, 태깅 | Alation, Apache Atlas | 도서관 색인 카드 |
...
└────────────┴───────────────────────┴───────────────────────┴────────────────────────┘
```
이 표의 핵심은 권한이 이동함에 따라 리스크의 성질도 변한다는 점이다. 과거 IT 주도 환경에서의 리스크가 '느린 속도'였다면, 셀프서비스 환경에서의 최대 리스크는 각 부서가 자기 방식대로 지표를 계산하여 "우리 팀 기준으로는 매출이 올랐다"고 주장하는 **'단일 진실 공급원(SSOT, Single Source of Truth)'의 붕괴**다. 
...
기술사는 반드시 ①쿼리 비용 상한선(Quota) 설정, ②대시보드 캐싱(Caching) 강제, ③장기 미사용 대시보드 자동 삭제라는 3중 방어막(Guardrail)을 설계해야 한다.
