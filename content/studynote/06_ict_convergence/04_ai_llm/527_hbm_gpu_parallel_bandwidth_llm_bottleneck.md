...
대형 언어 모델은 훈련 시 연산 집약적이지만, **추론 시에는 배치 크기가 작아 메모리 대역폭 병목**이 지배적이다. 70B 모델의 FP16 가중치만 약 140GB — 단일 GPU로는 적재조차 불가능하다.
...
| Memory Bandwidth Bound | 병목 유형 | 메모리 대역폭이 제한 요소 |
