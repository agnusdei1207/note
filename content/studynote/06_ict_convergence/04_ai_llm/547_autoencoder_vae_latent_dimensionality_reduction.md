...
2. **KL 가중치(β-VAE)**: β > 1이면 더 분리된(Disentangled) 잠재 공간 → 해석 가능성 향상
3. **VQ-VAE**: 연속 잠재 공간 대신 이산(Discrete) 코드북 → 언어 모델과 통합 용이 (DALL-E 1 기반)
...
| LDM | VAE 응용 | 잠재 공간 디퓨전 생성 |
| 이상 탐지 | AE 응용 | 재구성 오류 기반 |
