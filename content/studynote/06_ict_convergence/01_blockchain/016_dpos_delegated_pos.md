...
date = "2026-03-04"
[taxonomies]
tags = ["DPoS", "합의 알고리즘", "블록체인", "EOS", "위임 지분 증명", "거버넌스"]
...
|:---|:---|:---|:---|:---|
| **Stakeholder (유권자)** | 토큰 보유자로서 투표권 행사 | 지분(토큰)을 스마트 컨트랙트에 예치(Staking)하고 BP 후보에게 투표 | 스테이킹 컨트랙트 | 유권자 |
...
| **Voting System (거버넌스)** | 지속적인 순위 갱신 | 토큰 지분율을 가중치로 하여 실시간/주기적으로 BP의 득표수 합산 및 순위 변동 적용 | 투표 알고리즘 | 선거 관리 위원회 |
| **BFT Layer (선택적)** | 블록의 즉각적 확정성 부여 | BP들 간 2/3 이상의 서명을 통해 블록을 되돌릴 수 없는 상태로 즉시 확정 | pBFT, 텐더민트 | 의사봉 타격 |
...
| **블록 생성 속도** | 약 10분 (비트코인 기준) | 수 초 ~ 수십 초 | **0.5초 이내 (EOS 기준)** | 실시간 서비스 가능 여부 |
| **성능 (TPS)** | 7 ~ 15 TPS | 수백 ~ 수천 TPS | **수천 ~ 1만+ TPS** | 엔터프라이즈 도입 가능성 |
| **포크(Fork) 확률** | 매우 높음 (네트워크 지연 시) | 보통 | **거의 없음 (순번제)** | 합의 완결성(Finality) |
...
|:---|:---|:---|:---|
| **트랜잭션 비용** | 건당 수수료 폭등 (가스비 전쟁) | 자원 임대(Staking) 방식으로 무료화 가능 | 사용자 체감 비용 99% 절감 |
