...
| 개념 | 연결 포인트 |
|:---|:---|
| **해시 함수 (Hash Function)** | 각 노드 값을 만들고 상위 노드로 연결하는 기본 연산 |
| **머클 트리 (Merkle Tree)** | 거래 데이터를 계층적으로 묶어 무결성을 요약하는 구조 |
| **머클 루트 (Merkle Root)** | 전체 거래 집합을 대표하는 단일 해시값 |
| **SPV 경량 검증 (Simplified Payment Verification)** | 머클 증명으로 전체 블록 없이 거래 포함 여부를 확인하는 방식 |
