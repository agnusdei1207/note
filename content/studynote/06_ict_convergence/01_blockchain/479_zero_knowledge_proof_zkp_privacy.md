...
| **신원 인증** | ZK 로그인(Google OAuth+ZKP) | 개인정보 없이 인증 |
...
- **Shielded Transaction**: z-주소 간 거래 → 금액·발신자·수신자 모두 은닉
- **Transparent Transaction**: t-주소 간 거래 → 비트코인과 동일 공개
- **선택적 공개(Viewing Key)**: 특정 감사자에게만 거래 내역 공개 가능

### 기술사 핵심 판단
1. **Trusted Setup 리스크**: zk-SNARKs의 CRS(Common Reference String) 생성 시 비밀 파라미터 파기 실패 시 위조 증명 가능 → 다자 계산(MPC) Ceremony로 완화
2. **증명 생성 비용**: Prover의 연산이 매우 무거움 → 전용 하드웨어(FPGA, ASIC) 필요
3. **규제 충돌**: 프라이버시 코인은 FATF(Financial Action Task Force) Travel Rule 준수 어려움
4. **ZK-EVM 성숙도**: Polygon zkEVM, zkSync Era가 범용 EVM 증명 지원 단계 진입
