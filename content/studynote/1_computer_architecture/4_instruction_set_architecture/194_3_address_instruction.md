...
- **概念**: 3-주소 명령어는 '가장 친절하고 완벽한 지시문'이다. "누구(Src1)와 누구(Src2)를 어떻게(Opcode) 해서, 그 결과를 정확히 어디(Dest)에 두어라"라고 6하 원칙에 맞춰 명령한다. 생략된 것도, 숨겨진 암묵적 룰(Implied)도 없는 가장 명시적인 체계다.
- **💡 비유**: 3-주소 명령어는 '표준 레시피'와 같다. "밀가루(A)와 계란(B)을 섞어서, 오븐용 새 그릇(C)에 담아라." 밀가루 봉지와 계란 껍데기는 그대로 남아있고, 완성된 반죽은 완전히 새로운 그릇에 담기니 주방이 아주 깔끔하게 유지된다.
- **등장 배경**: 
  1980년대, x86 같은 2-주소 CISC 칩은 파괴적 대입 때문에 컴파일러 개발자들을 미치게 만들었다. "데이터가 자꾸 지워지니까 MOV로 복사하느라 코드가 너무 길어집니다!" 이때 버클리와 스탠퍼드의 연구진들(RISC의 창시자들)은 결단했다. "트랜지스터도 싸졌는데, 그냥 명령어 길이를 32비트로 넉넉하게 고정하고 주소를 3개나 파버리자!" 이렇게 원본을 보존하는 3-주소 명령어가 탄생하며, 칩 아키텍처의 패권은 RISC로 넘어가게 된다.

원본을 건드리지 않고 독립적인 결과를 만들어내는 3-주소의 우아한 메커니즘을 시각화하면 다음과 같다.
