...
- **概念**: 2-주소 명령어는 '재활용의 미학'이자 '파괴적 연산'이다. "A와 B를 더해서 C에 넣어라(3-주소)"라고 하는 대신, "A와 B를 더해서 **다시 A에 넣어라**(2-주소)"라고 명령한다. 결과값을 담을 새로운 주소(C)를 쓸 공간이 없으니, 원래 있던 재료(A)의 공간을 부수고 거기에 요리를 담는 것이다.
- **💡 비유**: 2-주소 명령어는 '커피잔에 우유 붓기'와 같다. 커피잔(A)과 우유통(B)이 있을 때, 굳이 새로운 큰 컵(C)을 가져오지 않는다. 그냥 우유(B)를 커피잔(A)에 부어버린다. 우유는 그대로 남지만, 원래의 순수했던 블랙커피(A)는 사라지고 라떼(A+B)로 완전히 덮어써지게 된다.
- **등장 배경**: 
  과거 1-주소(누산기) 머신은 레지스터가 하나뿐이라 병목 현상이 너무 심했다. 그래서 레지스터를 여러 개(GPR) 늘렸는데, 이번엔 주소를 3개나 적으려니 명령어 길이가 32비트를 훌쩍 넘어가서 메모리가 버티질 못했다. 공학자들은 **"보통 덧셈을 할 때 `i = i + 1`처럼 자기 자신에 덮어쓰는 경우가 90% 이상이잖아? 그럼 주소를 2개만 적고, 첫 번째 주소를 도착지로 재활용하자!"**라는 기가 막힌 타협점을 찾아냈다. 이것이 x86을 탄생시킨 2-주소 명령어의 기원이다.

"A + B"를 계산할 때 A의 원본을 무자비하게 덮어버리는 파괴적 메커니즘을 시각화하면 다음과 같다.
