...
- **概念**: 간접 주소 지정은 '쪽지 속의 쪽지'다. 보물찾기 쪽지를 열었더니 보물이 들어있는 게 아니라, "진짜 보물은 산 너머 500번지에 있다"라고 적힌 또 다른 쪽지(주소)가 들어있는 것과 같다. 실제 데이터를 손에 넣으려면 메모리를 최소한 두 번 방문해야 한다.
- **💡 비유**: 간접 주소 지정은 '친구의 이사 간 주소 물어보기'와 같다. 친구(데이터)를 만나러 예전 집(명령어 속 주소)에 갔더니, 문 앞에 "저 101호로 이사 갔어요"라고 새 주소가 적혀 있는 셈이다. 그 새 주소로 다시 찾아가야만 비로소 친구를 만날 수 있다.
- **등장 배경**: 
