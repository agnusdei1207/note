...
- **概念**: 더블 워드는 이름 그대로 '워드 두 개를 합친 크기'다. 컴퓨터 아키텍처가 16비트였던 시절에는 32비트가 더블 워드였고, 32비트 시대에는 64비트가 더블 워드가 되었다. 현재 우리가 쓰는 대부분의 PC 아키텍처(x86-64)에서는 64비트를 '쿼드 워드(Qword)'라 부르고 32비트를 '더블 워드'라 칭하는 등 세대에 따라 명칭이 융합되어 혼용되기도 한다.
