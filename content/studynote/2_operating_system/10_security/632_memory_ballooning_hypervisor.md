...
| **PFN (Page Frame Number)**| 할당/회수의 기준이 되는 물리 페이지 번호 | 게스트가 풍선에 준 메모리 주소 목록을 VMM과 공유 | 빈 공간 지도 |
...
| **최후 수단** | **Host Swapping (호스트 스와핑)**| 게스트의 의사와 무관하게 무자비하게 램을 디스크(SSD)로 내쫓음. | **Double Swapping 발생, 치명적 성능 저하** |

**Double Swapping (이중 스와핑) 이란?**
...
| **OOM (Out-Of-Memory) Killer** | 벌루닝이 한계에 달해 게스트 OS 내부의 메모리가 극도로 쪼들릴 때 게스트 커널이 프로세스를 죽이는 현상 |
