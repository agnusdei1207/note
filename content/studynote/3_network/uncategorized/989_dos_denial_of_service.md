...
description = "네트워크 및 시스템 자원을 고갈시켜 정상적인 서비스 제공을 방해하는 DoS 공격의 메커니즘과 방어 전략"
date = 2024-05-24
[taxonomies]
tags = ["Network", "Security", "DoS"]
+++

# 989. 서비스 거부 공격 (DoS)

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 서비스 거부 공격 (DoS, Denial of Service)은 타겟 시스템의 네트워크 대역폭, CPU (Central Processing Unit), 메모리, 커넥션 풀 등의 한정된 자원을 악의적인 트래픽으로 고갈시켜 정상적인 사용자의 서비스 접근을 차단하는 가용성 파괴 공격이다.
> 2. **가치**: 현대 인터넷 아키텍처에서 DoS 공격은 단일 노드 공격을 넘어 분산형인 DDoS (Distributed Denial of Service)로 진화했으며, 이를 방어하기 위해 클라우드 기반의 트래픽 스크러빙 센터 (Scrubbing Center)와 글로벌 CDN (Content Delivery Network) 에지 캐싱이 필수 인프라로 자리 잡았다.
> 3. **융합**: OSI (Open Systems Interconnection) 7계층 전반에 걸쳐 L3/L4의 볼륨 기반 공격과 L7의 애플리케이션 계층 공격으로 세분화되며, AI (Artificial Intelligence) 기반 이상 탐지 모델과 결합하여 동적인 공격 패턴을 실시간으로 차단하는 WAF (Web Application Firewall) 및 Anti-DDoS 솔루션으로 방어 체계가 고도화되고 있다.
...
2. **프로토콜 공격 (Protocol Attacks)**: 서버나 방화벽, 로드 밸런서 등 L3/L4 네트워크 인프라 장비의 실제 처리 능력(State Table 용량 등)을 소모시킨다. SYN Flooding, Ping of Death, Smurf Attack 등이 속한다.
...
### 비교 1: 기존 DoS vs DDoS vs DRDoS 구조 비교
...
### 비교 2: OSI 계층별 공격 기법 대비 (L4 vs L7)
...
2. **시나리오 — HTTP Slowloris 공격에 의한 웹 서버 커넥션 고갈**: 트래픽 볼륨은 지극히 정상 수준(수십 Kbps)이나, 웹 서버(Apache)의 최대 커넥션 수(MaxClients)가 계속 꽉 차 있어 정상 사용자의 웹 페이지 접속이 안 되는 상황. 분석 결과, 수백 개의 봇이 HTTP 헤더의 끝을 알리는 개행문자(`\r\n\r\n`)를 보내지 않고, 10초마다 의미 없는 헤더 필드를 한 줄씩 보내 커넥션을 끊지 않고 유지(Hold)하고 있음이 식별되었다.
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **DDoS (Distributed DoS)** | DoS 공격의 진화 형태로, 수많은 봇넷을 이용해 단일 타겟의 네트워크 대역폭과 자원을 분산 환경에서 압도적으로 고갈시킨다. |
| **봇넷 (Botnet) & C&C 서버** | 악성코드에 감염된 좀비 PC 군단으로, C&C (Command & Control) 서버의 명령에 따라 일제히 타겟을 향해 DoS 공격 트래픽을 쏟아내는 핵심 공격 인프라다. |
| **IP 스푸핑 (IP Spoofing)** | 패킷의 출발지 IP 주소를 조작하는 기법으로, DRDoS 증폭 공격에서 응답 트래픽을 타겟으로 유도하거나 공격자 추적을 회피하기 위해 필수적으로 사용된다. |
| **WAF (Web Application Firewall)** | L7 애플리케이션 계층에서 HTTP 요청의 페이로드를 검사하여 Slowloris나 GET Flooding 같은 정교한 서비스 거부 공격을 차단하는 핵심 필터링 역할을 한다. |
| **CDN (Content Delivery Network)** | 글로벌하게 분산된 엣지 서버를 통해 콘텐츠를 캐싱하여 오리진 서버의 부하를 줄일 뿐만 아니라, 방대한 대역폭으로 볼륨 기반 DDoS 트래픽을 일차적으로 흡수하는 방패 역할을 한다. |
| **BGP Blackholing / Anycast** | 네트워크 계층 방어 기법으로, 공격 트래픽이 몰리는 IP로 향하는 라우팅 경로를 무효화(Blackhole)하거나 우회시켜 스크러빙 센터에서 정제하도록 유도하는 네트워크 생존 기술이다. |
