...
description = "분산 컴퓨팅 자원을 통제하는 악성 인프라 봇넷과 명령 제어(C&C) 서버의 진화 아키텍처"
date = 2024-05-24
[taxonomies]
...
| **Bot (Zombie PC)** | 악성코드에 감염되어 실제 공격을 수행하는 말단 노드 | 주기적으로 C&C에 연결(Beacon)하여 명령 폴링 및 실행 | DNS, HTTP 커스텀 헤더 | 세뇌된 군단병 |
| **Dropper / Downloader** | 최초 감염 시 봇 악성코드를 타겟에 설치 | 이메일 첨부파일, 취약점 익스플로잇 후 페이로드 실행 | SMB, HTTP | 침투용 트로이 목마 |
| **DGA (Domain Generation Algorithm)** | C&C 도메인 차단 우회 메커니즘 | 시간/날짜 기반으로 매일 수천 개의 랜덤 도메인 동적 생성 | DNS | 끊임없이 바뀌는 비밀 암호 |

---

### 현대 봇넷의 진화된 아키텍처: P2P 봇넷과 DGA
...
### 비교 2: Beaconing 주기 특성과 탐지 회피 (네트워크 융합)
...
1. **시나리오 — 내부망에서 외부로 향하는 대량의 NXDOMAIN 질의 발생**: 사내 DNS 서버 로그 모니터링 중, 특정 PC 대역에서 존재하지 않는 도메인(예: `q1w2e3r4.com`)에 대한 해석(Resolution) 실패 로그(NXDOMAIN)가 초당 수십 건씩 폭증하는 현상 발견.
   - **의사결정**: 이는 전형적인 **DGA 기반 봇넷의 C&C 탐색 행위**다. 보안 엔지니어는 즉각 해당 내부 IP를 네트워크에서 격리(Isolate)하고 침해사고 대응(IR) 절차를 개시한다. 동시에 DNS 방화벽(또는 차세대 방화벽)을 설정하여 의미 없는 알파벳의 나열로 이뤄진 도메인을 식별하는 ML (Machine Learning) 기반의 DGA 탐지 모듈을 활성화한다. 단순 IP 차단으로는 해결되지 않으므로 엔드포인트 EDR (Endpoint Detection and Response)을 통해 악성코드 프로세스 자체를 격리해야 한다.
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **C&C (Command & Control)** | 봇넷 군단을 지휘하는 중앙 두뇌 시스템으로, 공격 명령 하달과 탈취된 데이터의 종착지 역할을 수행한다. |
| **DGA (Domain Generation Algorithm)** | 봇이 매일 수천 개의 랜덤한 C&C 도메인을 생성하여 방어자의 블랙리스트 차단을 교란하고 생존성을 극대화하는 회피 알고리즘이다. |
| **Sinkhole (싱크홀 라우팅)** | 악성 C&C 도메인에 대한 DNS 질의를 가짜 IP(분석 서버)로 우회시켜, 봇의 통신을 무력화하고 감염된 내부 IP 목록을 확보하는 방어 전술이다. |
| **P2P (Peer-to-Peer) 봇넷** | 중앙 C&C 서버 없이 감염된 봇들끼리 서로 연결되어 명령을 가십(Gossip)처럼 전파하는 아키텍처로, 단일 장애점(SPOF)을 제거해 파괴를 극도로 어렵게 만든다. |
| **DDoS (Distributed DoS)** | 봇넷이 C&C 서버의 명령을 받아 일제히 특정 타겟으로 트래픽을 쏟아부어 서비스를 마비시키는 가장 대표적이고 파괴적인 봇넷의 활용 사례다. |
| **Beaconing (비커닝)** | 감염된 봇이 C&C 서버에 주기적으로 신호를 보내 생존을 알리고 명령을 폴링(Polling)하는 통신 패턴으로, Jitter를 주어 탐지를 회피하려 시도한다. |
