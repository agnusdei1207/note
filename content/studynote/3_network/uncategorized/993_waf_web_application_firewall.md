...
weight = 993
title = "993. WAF (웹 방화벽)"
description = "웹 애플리케이션의 페이로드를 심층 분석하여 SQL Injection, XSS 등 L7 지능형 위협을 차단하는 보안 솔루션"
date = 2024-05-24
[taxonomies]
tags = ["Network", "Security", "WAF", "Web"]
+++

# 993. WAF (웹 방화벽)

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: WAF (Web Application Firewall, 웹 방화벽)는 일반적인 네트워크 방화벽이 통과시키는 80/443 포트 트래픽 내에서, HTTP/HTTPS 프로토콜의 페이로드(내용)를 심층 검사(DPI)하여 웹 애플리케이션 취약점을 노리는 악성 코드를 차단하는 OSI 7계층 전용 방어 솔루션이다.
> 2. **가치**: 기존 L4 방화벽과 IPS(Intrusion Prevention System)로는 방어할 수 없는 웹 특화 공격(SQL Injection, XSS, 무차별 대입 공격 등)을 사전에 차단하며, 특히 소스 코드 수정 없이 웹 서비스의 취약점을 임시 패치하는 가상 패치(Virtual Patching) 기능으로 무중단 비즈니스를 보장하는 핵심 인프라다.
> 3. **융합**: 전통적인 시그니처 기반의 정적 필터링을 넘어, AI (Artificial Intelligence)와 머신러닝(ML)을 결합해 자동화된 봇(Bot)의 비정상 스크래핑을 차단하고, MSA(Microservices Architecture) 환경에서 수많은 API(Application Programming Interface) 엔드포인트의 무결성을 검증하는 WAAP (Web Application and API Protection) 형태로 방어 패러다임이 융합·진화하고 있다.
...
|:---|:---|:---|:---|:---|
| **Reverse Proxy (리버스 프록시)** | 외부와 내부 사이의 통신 중계 및 격리 | 모든 클라이언트 요청을 대신 받아(TCP 종단) 검사 후 안전한 패킷만 웹 서버로 전달 | IP 은닉, 트래픽 버퍼링 | 왕의 음식 기미상궁 |
| **SSL/TLS Decryption (복호화)** | 암호화된 트래픽 안의 악성코드 확인 | 방화벽 내부에 인증서를 탑재하여 HTTPS 암호화를 풀고 평문 분석 수행 | 가시성 (Visibility) 확보 | 비밀 봉투 개봉 검사 |
| **Parsing Engine (파서)** | HTTP 메시지의 구조화 분해 | 헤더, URI, 쿠키, 본문(POST Data)을 규격에 맞게 분해 및 정규화(Normalization) | 우회/인코딩 공격 방어 | 폭발물 뇌관 해체 |
| **Detection Engine (탐지 엔진)** | 보안 룰셋 및 행위 분석 엔진 기반 평가 | 시그니처 매칭, 정규표현식 검사, 애플리케이션 프로파일링(ML) 대조 | SQLi, XSS, RFI 차단 핵심 | 지명수배자 얼굴 인식 |
| **Virtual Patching (가상 패치)** | 소스코드 수정 전 신속한 취약점 방어 | 특정 취약점(예: Log4j) 발표 시, 개발팀 패치 전 WAF 레벨에서 선제적 필터링 룰 적용 | 무중단 보안 운영 (Zero-downtime) | 구멍난 댐에 임시 합판 대기 |
...
1. **디코딩 및 정규화 (Normalization)**: 해커가 `<script>` 태그를 `%3Cscript%3E`로 인코딩하여 보내더라도, WAF 파서가 이를 평문으로 변환하여 본래의 의도를 파악한다. (우회 공격 방어의 핵심)
...
### 비교 1: WAF 구축 방식 (Architecture) 비교 및 트레이드오프
...
| **성능 오버헤드** | 전용 하드웨어 사용 (서버 부하 0) | 웹 서버의 CPU/메모리 직접 점유 | 클라우드 대역폭 자원 활용 (우수) | 백엔드 서버 처리량(TPS) 영향 |
...
### 비교 2: 주요 L7 웹 해킹 위협 벡터와 WAF 방어 기법 (OWASP Top 10)
...
|:---|:---|:---|:---|
| **정량** | 취약점 패치를 위해 서버 셧다운 (수 시간 소요) | WAF 가상 패치(Virtual Patching) 즉시 적용 | 제로데이 공격 시 서비스 **Downtime 제로화** 달성 |
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **SQL Injection (SQL 인젝션)** | 입력 폼에 악의적인 데이터베이스 쿼리문을 삽입하는 공격으로, WAF의 L7 페이로드 정규식 검사 기능이 이를 차단하는 1등 공신이다. |
| **XSS (Cross-Site Scripting)** | 게시판 등에 악성 자바스크립트를 숨겨 타인의 세션을 탈취하는 공격으로, WAF가 특수문자 인코딩 등을 해독하여 태그 삽입을 원천 차단한다. |
| **Virtual Patching (가상 패치)** | 소스코드의 취약점을 개발팀이 직접 수정하기 전, WAF 레벨에서 해당 공격 패턴을 인지하고 임시로 차단 룰을 적용해 보안 공백을 없애는 운영 전략이다. |
| **False Positive (오탐지)** | 정상적인 사용자의 웹 요청을 WAF가 공격으로 오인하여 차단하는 현상으로, WAF 도입 초기 투명 모드(Log-only)를 통한 예외 처리 튜닝이 필수적인 이유다. |
| **WAAP (Web App & API Protection)** | 전통적인 WAF의 기능에 Bot 관리, API 스키마 방어, L7 DDoS 차단 등을 통합하여 현대 클라우드 네이티브 환경을 보호하는 차세대 솔루션 개념이다. |
| **Bot Management (봇 관리)** | 검색 엔진 크롤러 같은 착한 봇은 통과시키고, 가격 스크래핑이나 계정 탈취(Credential Stuffing)를 시도하는 악성 봇은 CAPTCHA 등으로 기계적 접근을 막아내는 융합 기술이다. |
