...
weight = 936
title = "936. DNS 싱크홀"
description = "Zombie PC 봇넷 C&C 서버 질의 블랙리스트 감지 및 악성 도메인 무효화 처리"
date = 2024-05-20
[taxonomies]
...
### 비교 2: 전통적 DNS 질의 vs DoH (DNS over HTTPS) 환경에서의 싱크홀 한계
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **C&C 서버 (Command & Control)** | 봇넷 네트워크를 지휘하는 해커의 마스터 서버로, 싱크홀은 좀비 PC와 이곳의 논리적 연결을 절단한다. |
| **DGA (Domain Generation Algorithm)** | 악성코드가 탐지를 피하기 위해 매일 수천 개의 도메인을 랜덤 생성하는 기법으로, 싱크홀의 차단 목록 갱신 속도와 경합한다. |
| **Fast-Flux** | 하나의 도메인에 수백 개의 유동 IP를 짧은 TTL로 할당하여 방화벽의 IP 차단을 무력화하므로, 도메인 기반 싱크홀이 필수적인 이유가 된다. |
| **SIEM (Security Information & Event Management)** | 싱크홀에서 가로챈 악성 접속 시도 로그를 수집 및 상관 분석하여 내부 감염 호스트를 색출해내는 관제 중앙 두뇌다. |
| **DoH (DNS over HTTPS)** | DNS 질의를 평문이 아닌 HTTPS로 암호화하여 전송하므로, 기업망 중간에 위치한 기존 DNS 싱크홀을 무력화시키는 최신 위협 요소다. |
