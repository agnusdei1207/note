+++
weight = 992
title = "992. 방화벽 (Stateful Inspection)"
description = "단순한 패킷 필터링을 넘어 통신의 맥락(Context)을 추적하는 상태 기반 방화벽의 아키텍처와 진화"
date = 2024-05-24
[taxonomies]
...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 상태 기반 검사 (Stateful Inspection) 방화벽은 들어오고 나가는 네트워크 패킷을 독립적인 조각으로 보지 않고, 연결의 시작부터 종료까지 전체 세션의 '상태(State)'와 문맥을 메모리(State Table)에 기록하여 허용 여부를 결정하는 지능형(3세대) 보안 아키텍처다.
> 2. **가치**: 기존 1세대 패킷 필터링(Stateless)이 가졌던 규칙 복잡성(돌아오는 응답을 위해 포트를 무조건 열어둬야 하는 문제)을 해결했으며, 패킷 헤더 위조 공격(예: SYN 없이 들어오는 비정상 ACK 패킷)을 구조적으로 원천 차단하여 현대 엔터프라이즈 네트워크 방어의 사실상 표준(De facto standard)이 되었다.
> 3. **융합**: 연결의 맥락을 추적하는 이 L3/L4 계층의 세션 관리 기술은 애플리케이션 페이로드까지 분석하는 차세대 방화벽 (NGFW, Next-Generation Firewall)의 기반 엔진으로 융합되었으며, 최근에는 클라우드 인프라 내 마이크로서비스 간 트래픽(East-West)을 통제하는 분산 방화벽(Micro-segmentation)으로 패러다임이 진화하고 있다.
...
|:---|:---|:---|:---|:---|
| **ACL (Access Control List)** | 방화벽의 허용/차단 규칙 집합 | 첫 번째 패킷(세션 초기화) 인입 시 무거운 Top-Down 순차 검색 수행 | 보안 정책의 기준 법전 | 국경 통과 심사 매뉴얼 |
| **State Table (상태 테이블)** | 활성화된 통신 세션의 캐시 메모리 | 5-Tuple (Src/Dst IP & Port, Protocol) 및 연결 상태(TCP Flag) 저장 및 고속 해시 조회 | 트래픽 고속 우회 경로(Fast Path) | 발급된 프리패스 여권 |
| **Connection Tracker** | TCP 상태 전이 추적기 | 3-Way Handshake (SYN→SYN+ACK→ACK) 및 FIN/RST 종료 추적 | 통신 문맥 유지 | 행동 관찰 카메라 |
| **Timeout Timer (타이머)** | 끊어진 세션 정리 장치 | 트래픽이 없는 상태(Idle)가 일정 시간 지속되면 테이블에서 세션 강제 삭제 | 메모리 자원 고갈(DoS) 방어 | 도서관 대출 기한 시계 |
| **ALG (Application Layer Gateway)** | 다중 포트 프로토콜(FTP, SIP 등) 지원 | 제어 채널을 분석해 동적으로 데이터 채널 포트(Pinhole)를 열어줌 | 복잡한 앱의 방화벽 호환성 제공 | 귀빈을 위한 비밀 통로 개방 |
...
| **보안성** | 낮음 (IP 스푸핑, 비정상 TCP 플래그에 취약) | 매우 높음 (페이로드 내부 바이러스, 명령어 분석 가능) | 높음 (패킷 문맥 파악, 비정상 세션 차단) |
| **처리 성능** | 빠름 (단순 룰 매칭) | 매우 느림 (데이터를 모두 재조립해야 함, 병목 심각) | **매우 빠름** (State Table 기반 고속 패킷 바이패스) |
...
### 비교 2: Stateful 방화벽과 UDP/ICMP 통신 (Stateless 프로토콜의 처리)
...
|:---|:---|:---|:---|
| **정량** | 허용 룰(Rule) 증가 시 선형적인 성능 저하 | Fast Path 바이패스 적용으로 성능 저하 없음 | 대규모 룰 환경에서 **패킷 처리 지연(Latency) 수십 배 감소** |
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **Packet Filtering (패킷 필터링)** | OSI 3/4계층의 IP와 포트 번호만을 보고 정적으로 필터링하는 1세대 방화벽 기술로, Stateful 검사의 근간이 되는 기본 정책(ACL) 매칭 기능이다. |
| **TCP 3-Way Handshake (3방향 핸드셰이크)** | 통신 연결 수립을 위한 SYN, SYN-ACK, ACK 과정으로, Stateful 방화벽이 세션(State Table)을 생성하고 상태를 ESTABLISHED로 전환하는 핵심 기준점이다. |
| **NGFW (Next-Generation Firewall, 차세대 방화벽)** | Stateful 기능에 IPS(침입방지), L7 애플리케이션 제어, 평판 분석 등을 단일 엔진으로 융합하여 보안 심도를 7계층까지 확장한 진화형 장비다. |
| **ALG (Application Layer Gateway)** | FTP, SIP 등 데이터 전송 포트가 동적으로 변하는 다중 포트 프로토콜을 Stateful 환경에서 무사히 통과시키기 위해 방화벽이 애플리케이션 페이로드를 엿보고 핀홀(Pinhole)을 열어주는 보조 모듈이다. |
| **Asymmetric Routing (비대칭 라우팅)** | 패킷이 나가는 경로와 들어오는 경로가 다른 현상으로, 양방향 세션의 문맥을 모두 봐야 하는 Stateful 방화벽에서는 심각한 세션 차단 장애를 유발하는 운영상의 난제다. |
| **Micro-segmentation (마이크로 세그멘테이션)** | 경계 방어에 집중된 하드웨어 방화벽 기능을 클라우드 내부의 개별 가상 머신(VM) 단위로 분산시켜 세밀한 동서(East-West) 트래픽 통제를 구현하는 최신 보안 아키텍처다. |
