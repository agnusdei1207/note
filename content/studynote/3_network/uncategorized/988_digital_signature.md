...
weight = 988
title = "988. 전자 서명"
description = "전자 서명의 수학적 원리, 비대칭키와 해시의 결합 아키텍처 및 부인 방지 매커니즘 심층 분석"
date = 2024-05-24
[taxonomies]
tags = ["Network", "Security", "Digital Signature", "Cryptography", "Non-repudiation"]
categories = ["3_network"]
+++

# 전자 서명 (Digital Signature)

> ⚠️ 이 문서는 비대칭키 암호화와 해시 함수를 결합하여 디지털 세계에서 송신자의 신원 인증과 메시지의 무결성, 그리고 부인 방지를 수학적으로 달성하는 전자 서명의 아키텍처를 심층적으로 분석합니다.

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 전자 서명(Digital Signature)은 송신자가 자신의 '개인키(Private Key)'로 메시지의 해시값을 암호화한 데이터 블록으로, 수신자는 송신자의 '공개키(Public Key)'를 통해 이를 복호화하여 서명의 진위를 검증하는 알고리즘이다.
> 2. **가치**: 데이터가 전송 중 변조되지 않았음(무결성)을 보장할 뿐만 아니라, 오직 개인키 소유자만이 서명할 수 있다는 수학적 특성 덕분에 "내가 보내지 않았다"고 발뺌하는 것을 원천 차단하는 부인 방지(Non-repudiation)의 핵심 기술이다.
> 3. **융합**: 실무에서는 거대한 원본 데이터를 직접 서명하는 비효율을 없애기 위해 반드시 해시 함수(SHA-256 등)와 융합되어 사용되며, 이 서명값이 PKI(공개키 인프라)의 인증서와 결합하여 전 세계 인터넷 뱅킹과 전자 계약의 법적 효력을 뒷받침한다.
...
- **등장 배경 및 발전 과정**:
  ...
  3. **해시 함수와의 결합 (성능 문제 해결)**: 초창기에는 100페이지짜리 계약서 전체를 RSA로 암호화하려니 컴퓨터가 멈춰버렸다. 이를 해결하기 위해 문서 전체를 32바이트(해시)로 압축한 뒤 이 짧은 요약본에만 서명하는 방식이 고안되어 오늘날의 표준 아키텍처가 완성되었다.
...
### 비교 1: 전자 서명 vs 메시지 인증 코드 (MAC, HMAC)
...
- **운영체제 및 소프트웨어 공학**: 코드 사이닝(Code Signing). 애플이나 마이크로소프트는 앱 스토어에 올라오는 모든 실행 파일(`.exe`, `.apk`)에 개발자의 전자 서명을 강제한다. OS는 프로그램을 실행하기 전 서명을 검증하여, 변조된 악성코드이면 "게시자를 알 수 없거나 파일이 손상되었습니다"라며 실행을 OS 커널 단에서 차단한다.
...
2. **시나리오 — 난수 발생기(RNG) 고장으로 인한 ECDSA 개인키 유출**: 한 IoT 스타트업이 펌웨어 업데이트 검증을 위해 기기마다 ECDSA 전자 서명을 도입했다. 그런데 해커가 몇 개의 서명된 패킷만 가로채고도 서버의 개인키를 역산해 내는 대형 사고가 터졌다. 원인은 ECDSA 서명 생성 공식($s = k^{-1}(h + d \cdot r) \pmod n$)에 필수적으로 들어가는 임시 난수 $k$(Nonce)를, 난수 발생기가 고장 나 계속 똑같은 값으로 재사용했기 때문이다. 아키텍트는 서명 알고리즘 구현 시 하드웨어 기반 난수 발생기(TRNG)를 강제하거나, Nonce 재사용 공격에 면역인 결정론적(Deterministic) 서명 방식(EdDSA, RFC 6979)으로 암호화 라이브러리를 전면 교체해야 한다.
...
- **서명 검증 결과 무시**: 오픈소스 라이브러리 사용 시, 통신을 서둘러 붙이느라 `verify_signature=False` 형태의 플래그를 달아두고 운영 환경에 배포하는 행위. 전자 서명 아키텍처의 존재 이유 자체를 스스로 부정하는 가장 잦은 사고 원인이다.
...
| 개념 명칭 | 관계 및 시너지 설명 |
|:---|:---|
| **비대칭키 암호 (Asymmetric Key Cryptography)** | 전자 서명의 뼈대를 이루며, '개인키로 암호화하고 공개키로 복호화한다'는 역발상을 통해 서명 생성과 검증 체계를 제공한다. |
| **해시 함수 (Hash Function)** | 서명 속도를 높이기 위해, 방대한 원본 문서를 고정 길이(예: 256비트)로 압축하여 서명 알고리즘의 입력값으로 전달하는 필수 전처리 도구이다. |
| **X.509 인증서 (X.509 Certificate)** | 서명 검증 시 필요한 송신자의 '공개키'가 진짜 그 사람의 것인지 보증하기 위해, CA(인증기관)의 전자 서명으로 감싼 디지털 신분증이다. |
| **MAC / HMAC (Message Authentication Code)** | 대칭키(비밀키)를 사용하는 무결성/인증 기술로, 속도는 전자 서명보다 빠르나 개인키/공개키 쌍이 없어 부인 방지(Non-repudiation)를 제공하지 못한다. |
| **부인 방지 (Non-repudiation)** | 정보보안의 핵심 목표 중 하나로, 오직 개인키 소유자만이 올바른 전자 서명을 만들 수 있다는 성질을 통해 송신자가 송신 사실을 부인하지 못하게 막는다. |

---

## 👶 어린이를 위한 3줄 비유 설명
1. 인터넷에서 편지를 보낼 때 **'전자 서명'**을 딱 붙이면, 나만 알고 있는 '마법의 투명 도장'을 편지 전체에 찍어서 보내는 것과 같아요.
