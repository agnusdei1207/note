...
- [`[OSI 7계층 및 TCP/IP]`](@/studynote/3_network/1_datacomm_basics/_index.md): 네트워크의 모든 통신 과정을 설명하는 대원칙이자 아키텍처의 뼈대.
- [`[TCP 혼잡 제어 및 UDP]`](@/studynote/3_network/8_transport_layer_tcp_udp/_index.md): 시스템의 신뢰성과 속도를 결정짓는 4계층 프로토콜의 양대 산맥.
- [`[IP 라우팅 (OSPF/BGP)]`](@/studynote/3_network/6_network_layer_ip/_index.md): 최적의 경로를 동적으로 산출하는 네트워크 계층의 핵심 알고리즘.
- [`[네트워크 보안 (IPSec/TLS)]`](@/studynote/3_network/13_network_security_basics/_index.md): 도청 및 위변조를 막기 위한 암호화 터널링 기술.
- [`[SDN 및 클라우드 네이티브 망]`](@/studynote/3_network/17_sdn_nfv/_index.md): 하드웨어 종속성을 파괴하고 네트워크를 코드로 제어하는 현대 인프라의 종착점.